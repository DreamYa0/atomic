...
public class BaseInterfaceTest {
     ...
     * @throws Exception
     */
    @DataProvider(name = "httpExcel")
    public Iterator<Object[]> httpDataProvider(Method method) throws Exception {
        return readDataForTest(method);
    }
     ...
     * @throws Exception
     */
    @DataProvider(name = "parallelHttpExcel", parallel = true)
    public Iterator<Object[]> parallelHttpDataProvider(Method method) throws Exception {
        return readDataForTest(method);
    }
    ...
    @DataProvider(name = "httpCsv")
    public Iterator<Object[]> httpDataProviderFromCsv(Method method) {
        Set<Object[]> set = Sets.newLinkedHashSet();
        try {
            // 获取测试用例Excel文件路径
            String className = this.getClass().getSimpleName();
            String path = "src/test/resources/data/" + className + ".csv";
            File file = new File(path);
            if (!file.exists()) {
                throw new FileNotFoundException();
            }
            ...
            Reporter.log("[CommonTest#httpDataProviderFromCsv()]:{dataProvider处理失败！}");
            System.out.println("--------------dataProvider处理失败！----------------");
            e.printStackTrace();
            return set.iterator();
        }
